...
Purpose: the regex detectors in layers/risk.py catch known phrasings
verbatim ("ignore your instructions", "DAN mode", etc.) but miss any
paraphrase, synonym substitution, or novel wording that expresses the
same underlying intent. These exemplars are deliberately written to be
semantically equivalent to real attacks WITHOUT matching the regex
patterns already in risk.py — the goal is to cover the paraphrase gap,
not to duplicate what regex already catches.

Each category maps to the SAME flag name risk.py already emits, so the
semantic detector reinforces the existing taxonomy instead of inventing
a parallel one. Keep additions here paraphrase-diverse: prefer adding a
structurally different sentence over a near-duplicate of an existing one.
"""
