"""
Trains the logistic regression classifier that Layer 2 (layer2_entropy_engine.py)
uses to combine its four engineered features into a final cost_score.

Direct port of TwinShield's backend/ml/train_layer2_logistic_weights.py —
same synthetic-data generators, same feature extraction (calls the SAME
EntropyEngine methods the live engine uses, so there's no train/serve
skew), same sklearn Pipeline structure.

Usage:
    cd app
    python3 ml/train_layer2_logistic_weights.py
"""
...
from layer2_entropy_engine import EntropyEngine, LEET_MAP
...
def main():
    ...
    print(f"Synthesized dataset written to {csv_path}\n")
    ...
    joblib.dump(pipeline, ENTROPY_MODEL_PATH)
    print(f"\nModel saved to {ENTROPY_MODEL_PATH}")
