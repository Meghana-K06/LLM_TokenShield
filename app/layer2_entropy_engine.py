...
Direct port of TwinShield's backend/layers/entropy.py (EntropyEngine).
Four engineered signals, each scored 0-100:

  1. encoding_score      - base64/hex encoded payload detection
  2. expansion_factor     - amplification / token-bomb signal (1-50 scale,
                            normalized to 0-100 before scoring)
...
same as backend/ml/train_layer2_logistic_weights.py. Same formulas,
thresholds, and multiplier tables as your original — this is a port, not
a reimplementation.
"""
import re
import math
...
from config import ENTROPY_MODEL_PATH
...
# Minimal instruction-verb list. NOT one of the four scored signals —
# exists only to answer one yes/no question _estimate_expansion_factor
# needs: "is a number in this prompt paired with a command, or just an
# incidental number?" Without this gate, "the year 1000" and "write
# 1000 stories" would score identically.
...
class EntropyEngine:
    ...
    sending it to the LLM, via four engineered signals combined by a
    trained logistic regression classifier rather than a hand-picked
    weighted sum.
    ...
    format-grammar question, not something an ML model has any
    advantage learning. What moved to ML is the DECISION of how much
    each of the four signals should matter, not the low-level
    extraction of the signals themselves.
    """
    ...
    def __init__(self):
        ...
        try:
            self._model = joblib.load(ENTROPY_MODEL_PATH)
        except Exception:
            # No trained model on disk yet — falls back to a simple
            # average of the four signals so the layer never crashes,
            # but run ml/train_entropy_classifier.py to enable the real
            # scoring path.
            self._model = None
...
def entropy_defender_scan(payload: str) -> dict:
    """Adapter for main.py's pipeline: converts EntropyEngine's 0-100
    cost_score into the 0-1 scale used by Decision Fusion, and derives
    a SAFE/SUSPICIOUS/MALICIOUS label."""
    engine = _get_engine()
    result = engine.analyze(payload)
    cost_score = result["cost_score"]
    score_0_1 = cost_score / 100.0

    label = "MALICIOUS" if cost_score >= 70 else ("SUSPICIOUS" if cost_score >= 40 else "SAFE")
    ...
    return {
        ...
        "label": label,
        "reason": (
            f"cost_score={cost_score}/100 (encoding={b['encoding_score']:.0f} "
            f"expansion={b['expansion_factor']:.1f}x obfuscation={b['obfuscation_score']:.0f} "
            f"entropy={b['entropy_score']:.0f})"
        ),
        "details": {
            "cost_score": cost_score,
